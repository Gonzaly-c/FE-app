...
export function EstadoTrenFindAll() {
  //Obtiene todos los estados de trenes
  return useQuery({
...
export function EstadoTrenGetOne() {
  //Obtiene un estado de tren por ID
  return useMutation({
    mutationKey: ["estadoTrenGetOne"],
    mutationFn: async (estadoTrenId) => {
      console.log(`/estadoTren/${estadoTrenId}`);
      const estadoTren = await api.get(`/estadoTren/${estadoTrenId}`, {
...
export function EstadoTrenActivos() {
  //Obtiene todos los estados de trenes activos
  return useQuery({
