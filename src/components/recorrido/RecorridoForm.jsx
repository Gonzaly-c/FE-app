...
export function RecorridoForm({ onSuccess, recorridoToEdit }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-1">
        <label className="form-label" htmlFor="ciudadSalida">
          Ciudad Salida:
        </label>
        <input
          id="ciudadSalida"
          type="text"
          {...register("ciudadSalida", {
            required: 'La "Ciudad de salida" es requerida',
            value: recorridoToEdit ? recorridoToEdit.ciudadSalida : "",
          })}
          className="form-control"
          placeholder="Ciudad de salida del recorrido"
        />
        {errors.ciudadSalida && (
          <span className="text-danger">{errors.ciudadSalida.message}</span>
        )}
      </div>

      <div className="mb-1">
        <label className="form-label" htmlFor="ciudadLlegada">
          Ciudad Llegada:
        </label>
        <input
          id="ciudadLlegada"
          type="text"
          {...register("ciudadLlegada", {
            required: 'La "Ciudad de llegada" es requerida',
            value: recorridoToEdit ? recorridoToEdit.ciudadLlegada : "",
          })}
          className="form-control"
          placeholder="Ciudad de llegada del recorrido"
        />
        {errors.ciudadLlegada && (
          <span className="text-danger">{errors.ciudadLlegada.message}</span>
        )}
      </div>
      ...
      <div className="d-flex justify-content-between">
        ...
        <button
          ...
          style={{ backgroundColor: "#002050ff", ciudadLlegada: "#fff" }}
        >
      ...
      {isErrorPost && (
        <span className="text-danger">Error al crear el recorrido</span>
      )}
      {isErrorPut && (
        <span className="text-danger">Error al actualizar el recorrido</span>
      )}
    </form>
  );
}
