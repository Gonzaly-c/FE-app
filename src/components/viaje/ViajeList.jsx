...
export function ViajeList({
  ...
  const viajesFiltrados = viajes.filter((viaje) => {
    ...
    return (
      ...
      (!filtros.tren ||
        `${viaje.tren.modelo} ${viaje.tren.color}`
          .toLowerCase()
          .includes(filtros.tren.toLowerCase())) &&
      (!filtros.recorrido ||
        `${viaje.recorrido.ciudadSalida} ${viaje.recorrido.ciudadLlegada}`
          .toLowerCase()
          .includes(filtros.recorrido.toLowerCase())) &&
        ...
        <div className="row g-2">
          ...
          <div className="col">
            <input
              className="form-control"
              placeholder="Tren"
              onChange={(e) => setFiltros({ ...filtros, tren: e.target.value })}
            />
          </div>
          <div className="col">
            <input
              ...
              onChange={(e) =>
                setFiltros({ ...filtros, recorrido: e.target.value })
              }
            />
          </div>
          <div className="col">
            <input
              ...
              className="form-control"
              onChange={(e) =>
                setFiltros({ ...filtros, fechaIni: e.target.value })
              }
            />
          </div>
          <div className="col">
        ...
        <table className="table">
          <thead className="border-info fw-bold">
            <tr>
              ...
              <td className="text-center" style={{ paddingRight: 75 }}>
                Acción
              </td>
            </tr>
          </thead>
              ...
              <tr key={viaje.id}>
                ...
                <td className="text-center">
                  {viaje.conductor.nombre
                    ? viaje.conductor.nombre
                    : "Sin Conductor"}{" "}
                  {viaje.conductor.apellido ? viaje.conductor.apellido : ""}
                </td>
                <td className="text-center">
                  {viaje.tren.modelo ? viaje.tren.modelo : "Sin modelo"} (color:{" "}
                  {viaje.tren.color ? viaje.tren.color : "Sin color"})
                </td>
                <td className="text-center">
                  {viaje.recorrido.ciudadSalida
                    ? viaje.recorrido.ciudadSalida
                    : "Sin ciudad de salida"}{" "}
                  -
                  {viaje.recorrido.ciudadLlegada
                    ? viaje.recorrido.ciudadLlegada
                ...
                <td className="text-center">
                  {viaje.fechaIni
                    ? new Date(
                        new Date(viaje.fechaIni).getTime() + 3 * 60 * 60 * 1000
                ...
                <td className="text-center">
                  {viaje.fechaFin
                    ? new Date(
                        new Date(viaje.fechaFin).getTime() + 3 * 60 * 60 * 1000
                      ).toLocaleDateString("es-AR")
                    : "Sin fecha"}
                </td>
                <td className="text-center">
                ...
                <td className="text-center">
                  <button
                    ...
                    onClick={() =>
                      navigate(`/admin/lineaCargas?viajeId=${viaje.id}`)
                    }
                  >
                    Ver Cargas
                  </button>
                  <button
                    ...
                    style={{ backgroundColor: "#009e00ff", marginTop: "-10px" }}
                    onClick={() =>
                      navigate(`/admin/observaciones?viajeId=${viaje.id}`)
                    }
                  >
                    Ver Observaciones
                  </button>
