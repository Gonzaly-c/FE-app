...
export function LicenciaForm({ onSuccess, licenciaToEdit }) {
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      ...
      <div className="mb-1">
        <label className="form-label" htmlFor="fechaHecho">
          Fecha de hecho:
        </label>
        <input
          id="fechaHecho"
          type="date"
          {...register("fechaHecho", {
            required: 'La "Fecha de hecho" es requerida',
          })}
          className="form-control"
          placeholder="Fecha de hecho de la licencia"
          defaultValue={
            licenciaToEdit?.fechaHecho
              ? licenciaToEdit.fechaHecho.slice(0, 10)
              : ""
          }
        />
        {errors.fechaHecho && (
          <span className="text-danger">{errors.fechaHecho.message}</span>
        )}
      </div>

      <div className="mb-1">
        <label className="form-label" htmlFor="fechaVencimiento">
        ...
        <input
          ...
          {...register("fechaVencimiento", {
            ...
            validate: (value) => {
              ...
              return (
                fechaVencimiento >= fechaHecho ||
                'La "Fecha de vencimiento" debe ser posterior o igual a la de hecho'
              );
            },
          })}
          className="form-control"
          ...
          defaultValue={
            licenciaToEdit?.fechaVencimiento
              ? licenciaToEdit.fechaVencimiento.slice(0, 10)
              : ""
          }
        />
        {errors.fechaVencimiento && (
          <span className="text-danger">{errors.fechaVencimiento.message}</span>
        )}
      </div>
      ...
      {isErrorPost && (
        <span className="text-danger">Error al crear la licencia</span>
      )}
      {isErrorPut && (
        <span className="text-danger">Error al actualizar la licencia</span>
      )}
    </form>
  );
}
