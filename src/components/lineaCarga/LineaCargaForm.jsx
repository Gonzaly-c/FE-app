  ...
  useEffect(() => {
    if (!lineaCargaToEdit && idViaje && idCarga) {
      const existe = todasLasLineas.some(
        (lc) =>
          lc.viaje?.id === Number(idViaje) && lc.carga?.id === Number(idCarga)
      );
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)}>
      <div className="mb-1">
        ...
        >
          ...
          {viajes.map((c) => (
            <option key={c.id} value={c.id}>
              ...
              {c.fechaIni
                ? new Date(
                    new Date(c.fechaIni).getTime() + 3 * 60 * 60 * 1000
        ...
        {errors.idViaje && (
          <p style={{ color: "red" }}>{errors.idViaje.message}</p>
        )}
      </div>
      ...
      <div className="mb-1">
        ...
        {errors.idCarga && (
          <p style={{ color: "red" }}>{errors.idCarga.message}</p>
        )}
      </div>
      ...
      <div className="mb-1">
        ...
        <input
          ...
          {...register("cantidadVagon", {
            ...
            pattern: {
              value: /^[1-9][0-9]*$/,
              message:
                'La "Cantidad de vagones" debe ser un número entero mayor a 0',
            },
          })}
          className="form-control"
        ...
        {errors.cantidadVagon && (
          <p style={{ color: "red" }}>{errors.cantidadVagon.message}</p>
        )}
      </div>
      ...
      <div className="d-flex justify-content-between mt-3">
        ...
        <button
          ...
          disabled={isPendingForm || yaExisteError}
        >
      ...
      {isErrorPost && (
        <p style={{ color: "red" }} className="mt-2">
          Error al crear la línea de carga
        </p>
      )}
      {isErrorPut && (
        <p style={{ color: "red" }} className="mt-2">
          Error al actualizar la línea de carga
        </p>
      )}
    </form>
  );
