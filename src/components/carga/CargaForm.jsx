...
export function CargaForm({ onSuccess, cargaToEdit }) {
  ...
  const {
    register,
    formState: { errors, isSubmitting: isPendingForm },
    handleSubmit,
    reset,
  } = useForm({
    mode: "onBlur",
  ...
  return (
    <form onSubmit={handleSubmit(onSubmit)} noValidate>
      ...
      <div className="mb-3">
        ...
        {errors.idTipoCarga && (
          <span className="text-danger">{errors.idTipoCarga.message}</span>
        )}
      </div>
      {/* Nombre */}
      <div className="mb-1">
      ...
      {isErrorPost && (
        <span className="text-danger">Error al crear el tipoCarga</span>
      )}
      {isErrorPut && (
        <span className="text-danger">Error al actualizar el tipoCarga</span>
      )}
    </form>
  );
}
